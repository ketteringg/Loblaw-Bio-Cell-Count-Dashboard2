...
`part4_baseline_melanoma_samples.csv`, and `part4_summary.txt`. See
"Repository contents" below for what each one is. You do not strictly
have to run either first before the dashboard, though: `app.py`
...
| `part2_sample_total_counts.csv` | Part 2's per-sample total cell count step, shown as its own standalone file. See "Part 2: frequency table" below for how this relates to `percentage` |
...
`percentage` is computed from it. `part2_frequency_table.csv` already
contains this value (repeated once per population row, since it is one
row per sample-population pair), but `part2_sample_total_counts.csv`
surfaces it separately as its own file: one row per sample, just
`sample` and `total_count`, so that specific step is easy to check on
its own rather than needing to deduplicate the frequency table first.
Both files come from the same underlying computation
(`get_sample_totals()` reuses `get_frequency_table()` rather than
recomputing the sum a second way), so the two can never disagree.
