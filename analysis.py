...
def filter_dataset(
    ...
    subject/sample attribute in the schema: condition, treatment,
    sample_type, sex, project, response, time_from_treatment_start, age.
    """
    out = df
    if condition:
...
def get_filtered_frequency_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    percentage), but computed on an already-filtered cohort rather than
    the full dataset. Percentages are recalculated fresh within the
    filtered set (not re-derived from the unfiltered total), so they
    reflect that cohort's own composition.
    """
    if df.empty:
        return df
    out = df.copy()
    out["total_count"] = out.groupby("sample")["count"].transform("sum")
    out["percentage"] = out["count"] / out["total_count"] * 100
    return out[["sample", "total_count", "population", "count", "percentage"]]
