...
files are guaranteed to match what Custom Explorer / Cohort Comparison
would show for the equivalent fixed cohort, rather than being a
separately-maintained copy of the same logic.

The dashboard (app.py) is deliberately general-purpose (any cohort, any
comparison) and doesn't have fixed tabs for Parts 2/3/4 specifically, since
those are just special cases of what the dashboard can already do. This
script exists so the required, graded outputs still exist as concrete
files in the repo, without needing to click through the dashboard UI to
reconstruct the exact required filter combination.

Run after load_data.py:
    python generate_outputs.py

Writes to the repository root:
    frequency_table.csv           (Part 2)
    stats_results.csv             (Part 3)
    boxplot_responders.png        (Part 3)
    baseline_melanoma_samples.csv (Part 4.1)
...
def main() -> None:
    ...
    try:
        ...
        freq.to_csv(ROOT / "frequency_table.csv", index=False)
        print(f"Wrote frequency_table.csv ({len(freq)} rows)")

        # --- Part 3: stats + boxplot ---
        comparison = get_responder_comparison(conn)
        stats = run_stats_test(comparison)
        stats.to_csv(ROOT / "stats_results.csv", index=False)
        print(f"Wrote stats_results.csv ({len(stats)} rows)")

        # Plain matplotlib (not seaborn): the boxplot/stripplot combination
        # doesn't need an extra dependency for something matplotlib already
        # does natively, and keeping requirements.txt minimal means fewer
        # ways `make setup` can fail in a fresh Codespaces environment.
        fig, axes = plt.subplots(1, len(POPULATIONS), figsize=(3 * len(POPULATIONS), 5), sharey=True)
        rng = np.random.default_rng(0)  # fixed seed: reproducible output, not just visually similar each run
        ...
        fig.suptitle("Responder vs non-responder comparison (miraclib, PBMC)")
        fig.tight_layout()
        fig.savefig(ROOT / "boxplot_responders.png", dpi=150)
        plt.close(fig)
        print("Wrote boxplot_responders.png")
        ...
        baseline.to_csv(ROOT / "baseline_melanoma_samples.csv", index=False)
        print(f"Wrote baseline_melanoma_samples.csv ({len(baseline)} rows)")
