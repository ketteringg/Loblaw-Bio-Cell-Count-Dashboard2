...
This dashboard is deliberately general-purpose: two tabs, Custom Explorer
(any single cohort) and Cohort Comparison (any two cohorts side by side).
There are no fixed tabs for the assignment's Part 2/3/4 cohorts specifically
-- those are just special cases of what Custom Explorer / Cohort Comparison
can already produce by setting the equivalent filters, so a dedicated tab
for each would just be showing the same underlying data twice.

The required Part 2-4 outputs still exist as concrete files in the repo --
run `python generate_outputs.py` after `python load_data.py` to (re)produce
them. This dashboard is for open-ended exploration on top of that.
...
def render_boxplot(comparison_df: pd.DataFrame, stats_df: pd.DataFrame, x_col: str = "response_label"):
...
def render_cohort_comparison_boxplot(df_a: pd.DataFrame, df_b: pd.DataFrame, label_a: str, label_b: str, stats_df: pd.DataFrame):
    ...
    return render_boxplot(combined, stats_df, x_col="cohort")
...
def age_range_widgets(age_lo: int, age_hi: int, key_prefix: str) -> tuple[int, int] | None:
    ...
    sync via session_state -- dragging the slider updates the number
    inputs and vice versa. Returns the (min, max) tuple, or None if it
    still equals the full range (i.e. no restriction), matching
    filter_dataset's "None means no filter" contract.
    """
...
def cohort_filter_widgets(full: pd.DataFrame, key_prefix: str) -> dict:
    ...
    population_sel = st.multiselect(
        ...
        help="Leave empty to include all 5. Selecting specific populations "
             "only narrows which rows are shown/tested -- percentages "
             "still reflect each sample's full cell count, not just the "
...
st.info(
    "This dashboard is for open-ended exploration of any cohort. The "
    "assignment's required Part 2-4 outputs (frequency table, "
    "responders-vs-non-responders stats, baseline cohort breakdown) are "
    "generated as files in the repo by running `python generate_outputs.py` "
    "-- see frequency_table.csv, stats_results.csv, boxplot_responders.png, "
    "baseline_melanoma_samples.csv, and part4_summary.txt.",
    icon="📄",
)
...
with tab_explorer:
    ...
    filters = cohort_filter_widgets(full, "explorer")
    ...
    if filtered.empty:
        ...
        if avg_table.empty:
            ...
            st.dataframe(avg_table.rename(columns=display_cols), width='stretch', hide_index=True)
...
with tab_compare:
    ...
    full_cmp = cached_full_dataset(mtime)

    col_a, col_b = st.columns(2)
    with col_a:
        st.markdown("#### Cohort A")
        label_a = st.text_input("Label for Cohort A", value="Cohort A", key="label_a")
        filters_a = cohort_filter_widgets(full_cmp, "a")
    with col_b:
        st.markdown("#### Cohort B")
        label_b = st.text_input("Label for Cohort B", value="Cohort B", key="label_b")
        filters_b = cohort_filter_widgets(full_cmp, "b")

    cohort_a = filter_dataset(full_cmp, **filters_a)
    cohort_b = filter_dataset(full_cmp, **filters_b)

    st.divider()

    m1, m2, m3, m4 = st.columns(4)
    m1.metric(f"{label_a} — samples", cohort_a["sample"].nunique())
    m2.metric(f"{label_a} — subjects", cohort_a["subject_id"].nunique())
    m3.metric(f"{label_b} — samples", cohort_b["sample"].nunique())
    m4.metric(f"{label_b} — subjects", cohort_b["subject_id"].nunique())
    ...
    if status == "ok":
        ...
        if not avg_a.empty and not avg_b.empty:
            ...
            with chart_col1:
                fig_count = px.bar(
                    avg_combined, x="population", y="avg_count", color="cohort",
                    barmode="group", title="Average number of cells",
                    labels={"avg_count": "avg. cell count"},
                )
                st.plotly_chart(fig_count, width='stretch', key="compare_avg_count_chart")
            with chart_col2:
                fig_pct = px.bar(
                    avg_combined, x="population", y="avg_percentage", color="cohort",
                    barmode="group", title="Average relative frequency",
                    labels={"avg_percentage": "avg. % of total cells"},
                )
                st.plotly_chart(fig_pct, width='stretch', key="compare_avg_pct_chart")
